...
class ThreadlyASTTransformer {
  ...
  private createWorkerConfig(
    ...
    return {
      id: workerId,
      filePath: path.join(this.options.outputDir, `${workerId}.worker.ts`),
      functionName,
      annotation,
  ...
  private generateWorkerCode(config: WorkerConfig): string {
    ...
    workerCode += `// Generated by Threadly\n\n`;

    // Import the worker template - use a more reliable path
    workerCode += `import { exposeFunction } from 'threadly/dist/worker-template';\n`;
    ...
    // Expose the function using both Comlink and our template
    workerCode += `// Expose function for Comlink\n`;
    workerCode += `expose(${functionName});\n\n`;
    workerCode += `// Also expose using our template for direct execution\n`;
    workerCode += `exposeFunction('${functionName}', ${functionName});\n`;
